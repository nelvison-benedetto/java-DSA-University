...
public class Trie {
    ...
    List<Map<Character, Integer>> buildTrie(String[] patterns) {
        ...
        trie.add(new HashMap<>()); // root node (index 0)

        int newNode = 1; // next available index for a new node

        for (String pattern : patterns) {
            int currentNode = 0; // start from root
            for (char c : pattern.toCharArray()) {
                Map<Character, Integer> edges = trie.get(currentNode);
                if (edges.containsKey(c)) {
                    // edge already exists → go to child
                    currentNode = edges.get(c);
                } else {
                    // create new node
